...
Meson::Common::Memory::MemoryManager *GetBufferListMemoryManager(void);
...
template< class TElement>
class TBufferListEnumerator
	: public TEnumerator<TElement>
{
private:
	TBufferList<TElement>* m_pBufferList;
	size_t m_unIndex;
	bool* m_pbEnumerating;

public:
	TBufferListEnumerator(TBufferList<TElement>* p_pBufferList, bool* p_pbEnumerating)
		: m_pBufferList(p_pBufferList)
		, m_unIndex(0)
	...
	bool HasMoreElements(void)
	{
		...
		return m_unIndex < m_pBufferList->Size();
	}
	...
	TElement& NextElement(void)
	{
		...
		if( m_unIndex >= m_pBufferList->Size() )
			throw new MesonException( "No more elements to enumerate.", __FILE__, __LINE__ );

		return (*m_pBufferList)[m_unIndex++];
	}

	size_t Size( void ) const
	{
		return m_pBufferList->Size();
	}

	size_t Available( void ) const
	{
		return m_pBufferList->Size() - m_unIndex;
	}
};
...
protected:
	...
	float		m_fFactor;
	bool		m_bEnumerating;
	TBufferListEnumerator<TElement> m_bufferListEnumerator;


public: // Constructors and Destructor
	TBufferList( void )
		: m_bufferListEnumerator(this, &m_bEnumerating)
	{
		Init( 8 );
	}

	explicit TBufferList( int p_nSize )
		: m_bufferListEnumerator(this, &m_bEnumerating)
	{
		Init( p_nSize );
	}

	explicit TBufferList( size_t p_unSize )
		: m_bufferListEnumerator(this, &m_bEnumerating)
	{
		Init( (int)p_unSize );
	}

	TBufferList( const TImmutableList<TElement> &p_tList )
		: m_bufferListEnumerator(this, &m_bEnumerating)
	{
		m_fFactor = GROWTHFACTOR;
		Init( Growth( (int)p_tList.Size() ) );
		AddList( p_tList );
	}

	TBufferList( const TBufferList<TElement> &p_tBufferList )
		: m_bufferListEnumerator(this, &m_bEnumerating)
	{
		m_fFactor = GROWTHFACTOR;
		Init( (int)Growth( (int)p_tBufferList.Size() ) );
		AddList( p_tBufferList );
	}
	...
	{
		return GetBufferListMemoryManager()->AllocatePooled( p_unSize );
	}
...
public: // public ImmutableList interface methods
	...
	inline TEnumerator<TElement>& GetEnumerator(bool p_bReset = true)
	{
		if (p_bReset || !m_bEnumerating)
			m_bufferListEnumerator.Reset();
		m_bEnumerating = true;
		return m_bufferListEnumerator;
	}

	inline typename TEnumerator<TElement>::Ptr CreateEnumerator(void)
	{
		m_bEnumerating = true;
		return TEnumerator<TElement>::Ptr(new TBufferListEnumerator<TElement>(this, &m_bEnumerating));
	}
...
public: // public List interface methods
	...
	//------------------------------------------------------------------------------------------
	inline TBufferList<TElement>& operator=(const TList<TElement>& p_list)
	{
	...
	/// \returns a reference to this list.
	//------------------------------------------------------------------------------------------
	inline TBufferList<TElement>& operator=(const TBufferList<TElement>& p_list)
	{
		if( &p_list == this ) return *this;
	...
	/// \param p_list Reference to a list to append.
	//------------------------------------------------------------------------------------------
	inline void AddList(const TBufferList<TElement>& p_list)
	{
		size_t unSize = p_list.Size();
		EnsureCapacity( Growth( m_unSize + unSize ) );

		BufferCopyFwd( m_pBuffer + m_unSize, ((TBufferList<TElement>&)p_list).GetBuffer(), unSize );
		m_unSize += unSize;
	...
	//------------------------------------------------------------------------------------------
	inline void InsertList(size_t p_unIndex,
		const TBufferList<TElement>& p_list)
	{
		size_t unLength = p_list.Size();
		if( !unLength )
			return;
		...
		TEnumerator<TElement>& p_enumerator)
	{
		...
		{
			unLength++;
			p_enumerator.NextElement();
		}
		
		m_unSize += unLength;
		if( Growth( m_unSize ) <= (int)m_unBufferSize )
			Resize( Growth( m_unBufferSize ), p_unIndex, unLength );
		else
			Resize( Growth( m_unSize ), p_unIndex, unLength );
...
public: // public methods
	...
	inline TBufferList<TElement> operator+(const TElement& p_list)
	{
		TBufferList<TElement> lstResult( *this );
...
template<class TElement>
Meson::Common::Collections::TBufferList<TElement>
	operator+(
		const TElement &p_element,
		const Meson::Common::Collections::TBufferList<TElement> &p_list)
{
	Meson::Common::Collections::TBufferList<TElement> listResult(p_list);
