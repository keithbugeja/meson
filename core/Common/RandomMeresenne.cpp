...
const uint RandomMersenne::Period = 0xFFFFFFFF;
const uint RandomMersenne::m_unMagicNumbers[2] = { 0x0, MATRIX_A };
const double RandomMersenne::m_dfPeriodInverse = 1.0 / RandomMersenne::Period;

RandomMersenne::RandomMersenne( void ) 
{
	Seed(0x000C1CC0); 
}

void RandomMersenne::Seed( uint p_unSeed )
{
...
void RandomMersenne::Reset( void ) 
{ 
	Seed( m_unSeed ); 
}

uint RandomMersenne::Next( void ) 
{
	uint unResult;
...
uint RandomMersenne::Next( uint p_unRangeMax ) 
{
	return (uint)(m_dfPeriodInverse * p_unRangeMax * Next());
}

uint RandomMersenne::Next( uint p_unRangeMin, uint p_unRangeMax ) 
{
	return (uint)(m_dfPeriodInverse * (p_unRangeMax - p_unRangeMin) * Next()) + p_unRangeMin;
}

float RandomMersenne::NextFloat( void ) 
{
	return (float)(Next() * m_dfPeriodInverse);
}

double RandomMersenne::NextDouble( void ) 
{
